...
pathinC=strcat(glob_path,slash,'meteo_nc',slash,date,slash); % path wrf NetCDF data
    ...
    for day=dstart:dend % dstart=20180101; dend=20181231
        ...
        if strcmp(NAMEVAROUT{VAR},'aqh')
            ...
            eval(['d2m=ncread(''' pathinC ...
                datestr(day,'yyyymmdd') '00_arw_ecm_3km.nc'',''t2m'');'])
            eval(['msl=ncread(''' pathinC ...
                datestr(day,'yyyymmdd') '00_arw_ecm_3km.nc'',''msl'');'])
        ...
        elseif strcmp(NAMEVAROUT{VAR},'lwdown') % average value (since the beginning of the forecast)
            ...
            %for md=1:24
                eval(['varh=ncread(''' pathinC ...
                    datestr(day,'yyyymmdd') '00_arw_ecm_3km.nc'',''p3115'');'])
                %varm(:,:,md)=varh;
...
%            for md=1:24
                ...
                eval(['swdr=ncread(''' pathinC ...
                    datestr(day,'yyyymmdd') '00_arw_ecm_3km.nc'',''p3116'');'])
...
%             end
 ...
 %               if md==1
                    eval(['varh=ncread(''' pathinC ...
                        datestr(day,'yyyymmdd') '00_arw_ecm_3km.nc'',''p3059'');'])
%                     varc(:,:,md)=varh;
...
%             end
        ...
        elseif strcmp(NAMEVAROUT{VAR},'apress')
            clear var
            eval(['var=ncread(''' pathinC datestr(day,'yyyymmdd') '00_arw_ecm_3km.nc'',''msl'');'])
        ...
        elseif strcmp(NAMEVAROUT{VAR},'uwind')
            clear var
            eval(['uw=ncread(''' pathinC datestr(day,'yyyymmdd') '00_arw_ecm_3km.nc'',''u10'');'])
%            uw=uw(:,:,1:24);
            if (day ~= dend)
                uw=uw(:,:,1:24);
            elseif (day == dend)
...
%             uw=uw(:,:,1:24);
            eval(['vw=ncread(''' pathinC datestr(day,'yyyymmdd') '00_arw_ecm_3km.nc'',''v10'');'])
%            vw=vw(:,:,1:24);
            if (day ~= dend)
                vw=vw(:,:,1:24);
            elseif (day == dend)
...
%             end
        elseif strcmp(NAMEVAROUT{VAR},'atemp')
            clear var
            eval(['var=ncread(''' pathinC datestr(day,'yyyymmdd') '00_arw_ecm_3km.nc'',''t2m'');'])
%            var=var(:,:,1:24);
