function make_meteo_mitgcm_fc_mol(date)
...
glob_path2=strcat(slash,'ec',slash,'res4',slash,'hpcperm',slash,...
          'itai');
glob_path3=strcat(slash,'ec',slash,'res4',slash,'scratch',slash,...
          'itai');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
pathinC=strcat(glob_path3,slash,'meteo_nc',slash,date_year,slash); % path wrf NetCDF data
pathinM=strcat(glob_path3,slash,'meteo_txt',slash,date,slash); % path output
...
if exist(strcat(pathinC,daypath,'12_mol02ecm.nc'), 'file') == 2
    fileexist = strcat(pathinC,daypath,'12_mol02ecm.nc');
elseif exist(strcat(pathinC,daypath,'00_mol02ecm.nc'), 'file') == 2
    fileexist = strcat(pathinC,daypath,'00_mol02ecm.nc');
else
...
NAMEVAROUT={'aqh','lwflux','swflux','precip','apress',...
...
dend=datenum(date,'yyyymmdd')+8;
dstart = datenum(date,'yyyymmdd')-1;  %%%% Cambiare se si cambia il numero di giorni di analisi
...
for v=1:size(VAROUT,2)
    ...
    for day=dstart:dend % dstart=20180101; dend=20181231
        disp(day)
        fileexist = strcat(pathinC,datestr(day,'yyyymmdd'),'12_mol02ecm.nc');
        day_i = 0;
        if exist(fileexist, 'file') ~= 2
            if exist(strcat(pathinC,datestr(day+1,'yyyymmdd'),'00_mol02ecm.nc'),'file') == 2
                dayex=day+1;
                run_start='00';
                day_i = 1;
                disp('file non presente, uso giorno +1 alle 00')
            elseif exist(strcat(pathinC,datestr(day,'yyyymmdd'),'00_mol02ecm.nc'),'file') == 2
                dayex=day;
                run_start='00';
                day_i = 0;
                disp('file non presente, uso giorno +0 alle 00')
            elseif exist(strcat(pathinC,datestr(day-1,'yyyymmdd'),'12_mol02ecm.nc'),'file') == 2
                dayex=day-1;
                run_start='12';
                day_i = -1;
                disp('file non presente, uso giorno -1 alle 12')
            elseif exist(strcat(pathinC,datestr(day-1,'yyyymmdd'),'00_mol02ecm.nc'),'file') == 2
                dayex=day-1;
                run_start='00';
                day_i = -1;
                disp('file non presente, uso giorno -1 alle 00')
        ...
        if strcmp(NAMEVAROUT{VAR},'aqh')
            ...
            eval(['d2m=ncread(''' pathinC ...
                datestr(dayex,'yyyymmdd') run_start '_mol02ecm.nc'',''t2m'');'])
            eval(['msl=ncread(''' pathinC ...
                datestr(dayex,'yyyymmdd') run_start '_mol02ecm.nc'',''msl'');'])
            ...
            if (day_i == 0) && strcmp(run_start,'12')
                var=q(:,:,1+12:24+12);
            elseif (day_i == 1) && strcmp(run_start,'00')
                var=q(:,:,1:24);
            elseif (day_i == 0) && strcmp(run_start,'00')
                var=q(:,:,1+24:24+24);
            elseif (day_i == -1) && strcmp(run_start,'12')
                var=q(:,:,1+36:24+36);
            elseif (day_i == -1) && strcmp(run_start,'00')
                var=q(:,:,1+48:24+48);
            end
        ...
        elseif strcmp(NAMEVAROUT{VAR},'lwflux') % average value (since the beginning of the forecast)
            ...
            %for md=1:24
                eval(['varh=ncread(''' pathinC ...
                    datestr(dayex,'yyyymmdd') run_start '_mol02ecm.nc'',''p3112'');'])
                %varm(:,:,md)=varh;
...
%            for md=1:24
                ...
                eval(['swdr=ncread(''' pathinC ...
                    datestr(dayex,'yyyymmdd') run_start '_mol02ecm.nc'',''p3111'');'])
...
%             end
 ...
 %               if md==1
                    eval(['varh=ncread(''' pathinC ...
                        datestr(dayex,'yyyymmdd') run_start '_mol02ecm.nc'',''tp'');'])
%                     varc(:,:,md)=varh;
...
%            var=var/1000/3600; % from kg/m^2 (i.e., mm/hr) to m/s
        ...
        elseif strcmp(NAMEVAROUT{VAR},'apress')
            clear var
            eval(['var=ncread(''' pathinC datestr(dayex,'yyyymmdd') run_start '_mol02ecm.nc'',''msl'');'])
        ...
        elseif strcmp(NAMEVAROUT{VAR},'uwind')
            clear var
            eval(['uw=ncread(''' pathinC datestr(dayex,'yyyymmdd') run_start '_mol02ecm.nc'',''u10'');'])
%            uw=uw(:,:,1:24);
...
%             uw=uw(:,:,1:24);
            eval(['vw=ncread(''' pathinC datestr(dayex,'yyyymmdd') run_start '_mol02ecm.nc'',''v10'');'])
%            vw=vw(:,:,1:24);
...
%             end
        elseif strcmp(NAMEVAROUT{VAR},'atemp')
            clear var
            eval(['var=ncread(''' pathinC datestr(dayex,'yyyymmdd') run_start '_mol02ecm.nc'',''t2m'');'])
%            var=var(:,:,1:24);
